...
export const HabitProgressChart = React.memo(function HabitProgressChart({ completions, habitName }: HabitProgressChartProps) {
    // Show skeleton while loading or when no data
    ...
    export const TaskCard = React.memo(function TaskCard({ task, onToggleComplete, onEdit, onDelete }: TaskCardProps) {
        const isCompleted = task.status === 'completed'
        const priority = TASK_PRIORITIES[task.priority as TaskPriority] || TASK_PRIORITIES.medium
        const status = TASK_STATUS[task.status as TaskStatus] || TASK_STATUS.pending
        const category = task.category ? getCategoryById(task.category) : null
                ...
                <CardContent className="p-4">
                    <div className="flex items-start gap-3">
                        ...
                        <div className="pt-0.5">
                            <Checkbox
                                checked={isCompleted}
                                onCheckedChange={useCallback(() => onToggleComplete(task.id), [onToggleComplete, task.id])}
                                className="h-5 w-5"
                            />
                        </div>
                            ...
                            <div className="flex items-start justify-between gap-2">
                                ...
                                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <Button
                                        variant="ghost"
                                        size="sm"
                                        onClick={useCallback(() => onEdit(task), [onEdit, task])}
                                    ...
                                    <Button
                                        variant="ghost"
                                        size="sm"
                                        onClick={useCallback(() => onDelete(task.id), [onDelete, task.id])}
